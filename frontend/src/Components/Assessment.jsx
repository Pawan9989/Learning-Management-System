...
function Assessment() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8085/api/questions/${courseId}`)
      .then(res => res.json())
      .then(res => {
        setTest(res);
        setTotalQsns(res.length);
        setSelectedAnswers(new Array(res.length).fill("")); // Use empty string for each answer
      })
  ...
  const handleMarks = () => {
    ...
    const data = {
      ...
      marks: (correct / totalQsns) * 100
    };
    axios.post(`http://localhost:8085/api/assessments/add/${userId}/${courseId}`, data)
      .then(response => {
        ...
        {test.map((question, index) => (
          <div key={question.no} style={{ padding: "10px", backgroundColor: "rgb(454, 225, 180)", marginTop: "10px", borderRadius: "18px" }}>
            ...
            <label className="option">
              <input
                type="radio"
                name={`question_${index}`} // Use index for unique group
                value={question.option1}
                checked={selectedAnswers[index] === question.option1}
            ...
            <label className="option">
              <input
                ...
                checked={selectedAnswers[index] === question.option2}
            ...
            <label className="option">
              <input
                ...
                checked={selectedAnswers[index] === question.option3}
            ...
            <label className="option">
              <input
                ...
                checked={selectedAnswers[index] === question.option4}
